...
func (p *Bridge) ExecuteCommand(c *plugin.Context, args *model.CommandArgs) (*model.CommandResponse, *model.AppError) {
	...
	p.API.PublishWebSocketEvent(wsExitCommand, payload, &model.WebsocketBroadcast{UserId: args.UserId})
...
func (p *Bridge) handleInteract(w http.ResponseWriter, r *http.Request) {
	...
	p.API.PublishWebSocketEvent(wsExitInteract, map[string]any{"raw": raw}, &model.WebsocketBroadcast{})
