...
def test_react_dispatches_to_add_reaction():
    ...
    with patch("gateway.run._gateway_runner_ref", lambda: _runner_with(adapter)):
        result = _call(
            {"action": "react", "target": "photon:+155****4567", "message_id": "post_x", "emoji": "❤️"}
        )
    assert result["success"] is True
    assert adapter.calls == [("add", "+155****4567", "❤️", "post_x")]


def test_react_requires_message_id():
    adapter = _FakePhotonAdapter()
    with patch("gateway.run._gateway_runner_ref", lambda: _runner_with(adapter)):
        result = _call({"action": "react", "target": "photon:+155****4567", "emoji": "👍"})
    assert result.get("success") is not True
    assert "message_id" in json.dumps(result)


def test_react_without_live_gateway():
    with patch("gateway.run._gateway_runner_ref", lambda: None):
        result = _call(
            {"action": "react", "target": "photon:+155****4567", "message_id": "post_x", "emoji": "👍"}
        )
    assert result.get("success") is not True
